...
fn temp_database() -> (std::path::PathBuf, Database) {
    let path = std::env::temp_dir().join(format!("secrethub-vault-{}.sqlite", uuid::Uuid::new_v4()));
...
fn creates_secret_then_reopens_and_unlocks_without_returning_plaintext_metadata() {
    ...
    vault.create_secret(NewSecretInput {
        name: "OpenAI Main".into(), provider_id: "openai".into(), env_key: "OPENAI_API_KEY".into(),
        description: "local development".into(), tags: vec!["ai".into()], value: "fixture-secret-value".into(),
    }).unwrap();
...
fn updates_and_deletes_secret_through_the_vault_service() {
    ...
    let mut vault = VaultService::open(database).unwrap();
    vault.setup_master("correct horse battery staple").unwrap();
    let id = vault.create_secret(NewSecretInput { name: "Before".into(), provider_id: "generic".into(), env_key: "BEFORE".into(), description: String::new(), tags: vec![], value: "fixture-before".into() }).unwrap();
    vault.update_secret(&id, NewSecretInput { name: "After".into(), provider_id: "generic".into(), env_key: "AFTER".into(), description: "updated".into(), tags: vec!["changed".into()], value: "fixture-after".into() }).unwrap();
