...
fn redacts_sensitive_key_value_pairs_without_touching_safe_metadata() {
    let result = redact_text("provider=OpenAI api_key=fixture-value status=valid token:fixture-token");
...
fn redacts_sensitive_json_fields_recursively() {
    let result = redact_json(json!({"provider":"OpenAI","api_key":"fixture-value","nested":{"cookie":"fixture-cookie"}}));
