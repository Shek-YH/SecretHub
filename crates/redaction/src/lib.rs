...
const SENSITIVE_FIELDS: &[&str] = &[
    "api_key", "apikey", "token", "password", "secret", "authorization", "cookie", "private_key",
];

fn is_sensitive(name: &str) -> bool {
    let normalized = name.trim_matches(|c: char| !c.is_ascii_alphanumeric() && c != '_').to_ascii_lowercase();
...
pub fn redact_text(input: &str) -> String {
    input.split_whitespace().map(|token| {
        ...
        let Some(index) = separator else { return token.to_owned(); };
