...
fn initializes_vault_and_round_trips_metadata_and_encrypted_payload() {
    ...
    let id = database.create_secret(NewSecret {
...
fn database_does_not_store_the_plaintext_fixture() {
    ...
    database.create_secret(NewSecret {
        name: "Fixture".into(), provider_id: "generic".into(), env_key: "FIXTURE_KEY".into(),
        description: String::new(), tags: vec![], ciphertext: b"ciphertext-only".to_vec(), nonce: vec![0; 12],
    }).unwrap();
    let connection = Connection::open(database.path()).unwrap();
    let contents: String = connection.query_row("SELECT quote(ciphertext) FROM secret_payloads LIMIT 1", [], |row| row.get(0)).unwrap();
...
fn audit_records_keep_only_hashes_and_allowlisted_metadata() {
    let (_path, database) = temp_database();
    database.record_audit("create_secret", Some("secret-1"), Some("sha256:fixture"), "success", r#"{"provider":"openai"}"#).unwrap();
...
fn persists_profiles_projects_and_safe_default_settings() {
    let (_path, database) = temp_database();
    let now = 10;
    database.save_profile(&secrethub_storage::StoredProfile { id: "profile-ai".into(), name: "AI Standard".into(), description: "default AI stack".into(), secret_ids: vec!["secret-a".into(), "secret-b".into()], created_at: now, updated_at: now }).unwrap();
    assert_eq!(database.list_profiles().unwrap()[0].secret_ids, vec!["secret-a", "secret-b"]);
    let project = database.record_project("C:\\Projects\\demo", "demo").unwrap();
...
fn updates_and_deletes_secret_payload_by_id() {
    let (_path, database) = temp_database();
    let id = database.create_secret(NewSecret { name: "Before".into(), provider_id: "generic".into(), env_key: "BEFORE".into(), description: String::new(), tags: vec![], ciphertext: vec![1], nonce: vec![0; 12] }).unwrap();
    assert!(database.update_secret(&id, secrethub_storage::UpdatedSecret { name: "After".into(), provider_id: "generic".into(), env_key: "AFTER".into(), description: "updated".into(), tags: vec!["changed".into()], ciphertext: vec![2], nonce: vec![1; 12] }).unwrap());
