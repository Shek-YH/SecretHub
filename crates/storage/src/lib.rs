...
pub struct UpdatedSecret { pub name: String, pub provider_id: String, pub env_key: String, pub description: String, pub tags: Vec<String>, pub ciphertext: Vec<u8>, pub nonce: Vec<u8> }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct AuditEvent { pub id: String, pub operation: String, pub secret_id: Option<String>, pub value_hash: Option<String>, pub result: String, pub created_at: i64, pub metadata_json: String }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct StoredProfile { pub id: String, pub name: String, pub description: String, pub secret_ids: Vec<String>, pub created_at: i64, pub updated_at: i64 }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ProjectRecord { pub id: String, pub path: String, pub display_name: String, pub last_used_at: i64 }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct Settings { pub locale: String, pub theme: String, pub auto_lock_minutes: u32, pub clipboard_timeout_seconds: u32, pub require_auth_before_export: bool, pub require_auth_before_reveal: bool }
...
impl Database {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, StorageError> {
        ...
        Ok(Self { connection, database_path })
    }

    pub fn path(&self) -> &Path { &self.database_path }
    ...
    pub fn get_vault_meta(&self) -> Result<Option<(Vec<u8>, Vec<u8>)>, StorageError> {
        self.connection.query_row("SELECT salt, verifier FROM vault_meta WHERE id = 1", [], |row| Ok((row.get(0)?, row.get(1)?))).optional().map_err(Into::into)
    }

    pub fn create_secret(&self, secret: NewSecret) -> Result<String, StorageError> {
        let id = format!("secret-{}", uuid::Uuid::new_v4());
        let now = unix_time();
        let tags_json = serde_json::to_string(&secret.tags).map_err(|error| StorageError::InvalidData(error.to_string()))?;
        self.connection.execute(
        ...
        self.connection.execute("INSERT INTO secret_payloads(secret_id, ciphertext, nonce) VALUES (?1, ?2, ?3)", params![id, secret.ciphertext, secret.nonce])?;
        Ok(id)
    }
    ...
    pub fn list_metadata(&self) -> Result<Vec<SecretMetadata>, StorageError> {
        ...
        let rows = statement.query_map([], metadata_from_row)?.collect::<Result<Vec<_>, _>>()?;
        Ok(rows)
    }
    ...
    pub fn search_metadata(&self, query: &str) -> Result<Vec<SecretMetadata>, StorageError> {
        ...
        let rows = statement.query_map(params![pattern], metadata_from_row)?.collect::<Result<Vec<_>, _>>()?;
        Ok(rows)
    }

    pub fn get_payload(&self, id: &str) -> Result<Option<(Vec<u8>, Vec<u8>)>, StorageError> {
        self.connection.query_row("SELECT ciphertext, nonce FROM secret_payloads WHERE secret_id = ?1", params![id], |row| Ok((row.get(0)?, row.get(1)?))).optional().map_err(Into::into)
    }

    pub fn delete_secret(&self, id: &str) -> Result<bool, StorageError> {
        Ok(self.connection.execute("DELETE FROM secrets WHERE id = ?1", params![id])? > 0)
    }
    ...
    pub fn update_secret(&self, id: &str, secret: UpdatedSecret) -> Result<bool, StorageError> {
        ...
        self.connection.execute("UPDATE secret_payloads SET ciphertext = ?1, nonce = ?2 WHERE secret_id = ?3", params![secret.ciphertext, secret.nonce, id])?;
        Ok(true)
    }

    pub fn record_audit(&self, operation: &str, secret_id: Option<&str>, value_hash: Option<&str>, result: &str, metadata_json: &str) -> Result<(), StorageError> {
    ...
    pub fn list_audit(&self) -> Result<Vec<AuditEvent>, StorageError> {
        ...
        let rows = statement.query_map([], |row| Ok(AuditEvent { id: row.get(0)?, operation: row.get(1)?, secret_id: row.get(2)?, value_hash: row.get(3)?, result: row.get(4)?, created_at: row.get(5)?, metadata_json: row.get(6)? }))?.collect::<Result<Vec<_>, _>>()?;
        Ok(rows)
    }
    ...
    pub fn save_profile(&self, profile: &StoredProfile) -> Result<(), StorageError> {
        ...
        self.connection.execute("DELETE FROM profile_secrets WHERE profile_id = ?1", params![profile.id])?;
        for (position, secret_id) in profile.secret_ids.iter().enumerate() { self.connection.execute("INSERT INTO profile_secrets(profile_id, secret_id, position) VALUES (?1, ?2, ?3)", params![profile.id, secret_id, position as i64])?; }
        Ok(())
    }
    ...
    pub fn list_profiles(&self) -> Result<Vec<StoredProfile>, StorageError> {
        ...
        let rows = statement.query_map([], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?, row.get::<_, String>(2)?, row.get::<_, i64>(3)?, row.get::<_, i64>(4)?)))?.collect::<Result<Vec<_>, _>>()?;
        for (id, name, description, created_at, updated_at) in rows { let mut ids = self.connection.prepare("SELECT secret_id FROM profile_secrets WHERE profile_id = ?1 ORDER BY position")?.query_map(params![id], |row| row.get(0))?.collect::<Result<Vec<String>, _>>()?; profiles.push(StoredProfile { id, name, description, secret_ids: std::mem::take(&mut ids), created_at, updated_at }); }
        Ok(profiles)
    }

    pub fn record_project(&self, path: &str, display_name: &str) -> Result<ProjectRecord, StorageError> {
        ...
        self.connection.query_row("SELECT id, path, display_name, last_used_at FROM projects WHERE path = ?1", params![path], |row| Ok(ProjectRecord { id: row.get(0)?, path: row.get(1)?, display_name: row.get(2)?, last_used_at: row.get(3)? })).map_err(Into::into)
    }

    pub fn list_projects(&self) -> Result<Vec<ProjectRecord>, StorageError> {
        let mut statement = self.connection.prepare("SELECT id, path, display_name, last_used_at FROM projects ORDER BY last_used_at DESC")?;
        let projects = statement.query_map([], |row| Ok(ProjectRecord { id: row.get(0)?, path: row.get(1)?, display_name: row.get(2)?, last_used_at: row.get(3)? }))?.collect::<Result<Vec<_>, _>>()?;
        Ok(projects)
    }

    pub fn get_settings(&self) -> Result<Settings, StorageError> {
        self.connection.execute("INSERT OR IGNORE INTO settings(id) VALUES (1)", [])?;
...
fn metadata_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<SecretMetadata> {
    ...
    let tags = serde_json::from_str(&tags_json).map_err(|error| rusqlite::Error::FromSqlConversionFailure(5, rusqlite::types::Type::Text, Box::new(error)))?;
    Ok(SecretMetadata { id: row.get(0)?, name: row.get(1)?, provider_id: row.get(2)?, env_key: row.get(3)?, description: row.get(4)?, tags, status: row.get(6)?, created_at: row.get(7)?, updated_at: row.get(8)? })
}

fn unix_time() -> i64 { std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() as i64 }
