...
pub fn validate_endpoint(provider_id: &str, endpoint: &str, _registry: &[ProviderDefinition]) -> Result<Url, String> {
    let url = Url::parse(endpoint).map_err(|_| "invalid endpoint URL".to_owned())?;
    if url.scheme() != "https" { return Err("provider validation requires HTTPS".to_owned()); }
    let host = url.host_str().ok_or_else(|| "endpoint host is required".to_owned())?;
    let expected = match provider_id { "openai" => "api.openai.com", "anthropic" => "api.anthropic.com", "gemini" => "generativelanguage.googleapis.com", "deepseek" => "api.deepseek.com", "xai" => "api.x.ai", "openrouter" => "openrouter.ai", "github" => "api.github.com", "supabase" => "api.supabase.com", "cloudflare" => "api.cloudflare.com", _ => return Err("provider has no network validator".to_owned()) };
    if host != expected || url.port().is_some() { return Err("endpoint is outside the fixed provider origin".to_owned()); }
    if host.parse::<IpAddr>().map(is_private_ip).unwrap_or(false) { return Err("private network endpoints are not allowed".to_owned()); }
    Ok(url)
}

fn is_private_ip(ip: IpAddr) -> bool {
    match ip { IpAddr::V4(value) => value.is_private() || value.is_loopback() || value.is_link_local() || value.is_unspecified(), IpAddr::V6(value) => value.is_loopback() || value.is_unique_local() || value.is_unicast_link_local() || value.is_unspecified() }
}
