...
use secrethub_exporter::{ensure_gitignore_env, env_is_tracked, gitignore_has_env, render_env, render_example, validate_export_directory, write_atomic, EnvEntry};
...
#[tauri::command]
pub fn setup_master(password: String, state: State<'_, AppState>) -> Result<(), String> { vault_from_state(&state)?.as_mut().ok_or_else(|| "vault unavailable".to_owned())?.setup_master(&password).map_err(|error| error.to_string()) }

#[tauri::command]
pub fn unlock(password: String, state: State<'_, AppState>) -> Result<(), String> { vault_from_state(&state)?.as_mut().ok_or_else(|| "vault unavailable".to_owned())?.unlock(&password).map_err(|error| error.to_string()) }

#[tauri::command]
pub fn lock(state: State<'_, AppState>) -> Result<(), String> { if let Some(vault) = vault_from_state(&state)?.as_mut() { vault.lock(); } Ok(()) }

#[tauri::command]
pub fn secret_list(query: Option<String>, state: State<'_, AppState>) -> Result<Vec<secrethub_storage::SecretMetadata>, String> { let vault = vault_from_state(&state)?; let vault = vault.as_ref().ok_or_else(|| "vault unavailable".to_owned())?; query.filter(|value| !value.is_empty()).map_or_else(|| vault.list_metadata(), |value| vault.search_metadata(&value)).map_err(|error| error.to_string()) }

#[tauri::command]
pub fn secret_create(request: SecretCreateRequest, state: State<'_, AppState>) -> Result<String, String> { vault_from_state(&state)?.as_ref().ok_or_else(|| "vault unavailable".to_owned())?.create_secret(NewSecretInput { name: request.name, provider_id: request.provider_id, env_key: request.env_key, description: request.description, tags: request.tags, value: request.value }).map_err(|error| error.to_string()) }

#[tauri::command]
pub fn secret_update(id: String, request: SecretCreateRequest, state: State<'_, AppState>) -> Result<(), String> { vault_from_state(&state)?.as_ref().ok_or_else(|| "vault unavailable".to_owned())?.update_secret(&id, NewSecretInput { name: request.name, provider_id: request.provider_id, env_key: request.env_key, description: request.description, tags: request.tags, value: request.value }).map_err(|error| error.to_string()) }

#[tauri::command]
pub fn secret_delete(id: String, state: State<'_, AppState>) -> Result<bool, String> { vault_from_state(&state)?.as_ref().ok_or_else(|| "vault unavailable".to_owned())?.delete_secret(&id).map_err(|error| error.to_string()) }
...
#[tauri::command]
pub fn secret_reveal(id: String, state: State<'_, AppState>) -> Result<(), String> { let value = vault_from_state(&state)?.as_ref().ok_or_else(|| "vault unavailable".to_owned())?.read_secret(&id).map_err(|error| error.to_string())?; rfd::MessageDialog::new().set_title("SecretHub").set_description(&value).set_buttons(rfd::MessageButtons::Ok).show(); Ok(()) }
...
#[tauri::command]
pub fn settings_update(settings: secrethub_storage::Settings, state: State<'_, AppState>) -> Result<(), String> { vault_from_state(&state)?.as_ref().ok_or_else(|| "vault unavailable".to_owned())?.update_settings(settings).map_err(|error| error.to_string()) }
