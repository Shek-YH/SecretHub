...
function ProviderPanel({ t, onConfigure }: { t: ReturnType<typeof getTranslation>; onConfigure: (preset: CredentialPreset) => void }) {
  ...
  return <section className="settings-panel provider-panel"><div className="settings-heading"><span className="section-kicker">PROVIDER DIRECTORY / TEMPLATE CATALOG</span><h2>{t.nav.providers}</h2><p>集中查看全部服务商、可用模型和请求地址；每张卡片都可以直接进入预填配置表单。</p></div><div className="provider-directory-grid">{providerTemplates.map((provider) => <article className="provider-directory-card" key={provider.id}><div className="provider-directory-heading"><div><strong>{provider.name}</strong><span>{provider.models.length} 个模型模板</span></div><span className="provider-dot green" /></div><div className="provider-models">{provider.models.slice(0, 4).map((model) => <span key={model.id}>{model.label}</span>)}{provider.models.length > 4 && <span>+ {provider.models.length - 4} 个</span>}</div><div className="provider-directory-url">{provider.baseUrl || '自定义请求地址'}</div><div className="provider-card-actions"><button className="secondary-button" onClick={() => configure(provider)}>配置</button>{provider.keyUrl && <button className="inline-link" onClick={() => void openWebsite(provider.keyUrl)}>API Key 官网 ↗</button>}</div></article>)}</div><div className="form-security"><span>◆</span>API Key 只会进入加密 Vault；配置表单仍可修改模型和请求地址。</div></section>;
}
